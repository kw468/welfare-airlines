...
def calcAPDvalue(f,ER,gamma,qBar,b,pvec):
    pstar               = jnp.array([pvec[0]]*39+[pvec[1]]*7+[pvec[2]]*7 + [pvec[3]]*4 + [pvec[4]]*3 )
    ...
    for t in range(1,T+1):
        ...
        if t != T: #update expected value function, this is for not the last period
            g               = jnp.array(f[:,:,-t-1,:,b])
            g               = g.at[r,c,:].set(g[r,r-c,:])
            EV              = EV.at[:,-t].set(jnp.sum(g[:,:,pstar[-t]]*V[:,-t][None,:],axis=1))
    return [pstar,V[:,0]]
...
def storeSim(Store_fl, CCP, pu, pa, ps, p0, gamma, mu, prices,VAR):
    ...
    for t in range(60):
        ...
        # output is sales, CS_L, CS_B, CS_ALL
        if pois > 0:        
...
if __name__ == "__main__":
    ...
    with Pool(processes=numThreads) as pool2:
        results             = np.array(pool2.map(partial(storeSim, CCP=CCP, pu=pu, pa=pa, ps=ps, p0=p0, gamma=gamma, mu=mu, prices=prices,VAR=VAR), StoreX))
        pool2.close()
        pool2.join()
