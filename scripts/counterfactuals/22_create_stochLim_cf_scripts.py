"""
    Create sh programs to run sl_sim_cf on SLURM.
--------------------------------------------------------------------------------
change log:
...
#scp -pr estimation/ kw468@bfg:/home/kw468/Projects/airlines_jmp/
...
for i in mkts:
    script = f"""#!/bin/bash
#SBATCH --job-name={i}_sl_cf
#SBATCH --output={i}_sl_cf.out
...
python -u sl_sim_cf.py {i}
"""
    f = open("runSLCF" + i + ".sh", "w")
    f.write(script)
    f.close()
