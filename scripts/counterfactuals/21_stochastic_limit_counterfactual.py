...
def poissonProcess(R,aB,bL,bB,prices,p,remainCap):
    ...
    if remainCap == 0:
        ...
        cs              = (-1/((1-R)*bL + R*bB))*np.log( 1 + np.exp(aB + ((1-R)*bL + R*bB)*prices[p]))
        if QD.sum() <= remainCap:
...
def create_stochLimit_plot(df,a):
    ...
    #
    df3 = df.loc[df.initC != 0].loc[df.initC == int(a)-6].groupby("t").priceD.mean()
    df4 = df.loc[df.initC != 0].loc[df.initC == int(a)].groupby("t").priceD.mean()
    df5 = df.loc[df.initC != 0].loc[df.initC == dif01].groupby("t").priceD.mean()
    #
    csfont                      = {'fontname':"Liberation Serif", 'fontsize':20}
    palette                     = ["#FF6700", "#FCB07E", "#6B717E", "#3A6EA5", "#004E98", "#070707"]
    sns.set(style="white",color_codes=False)
    fig                     = plt.figure(figsize=(2*1.5*6.4, 1.1*4.8))
    ax1 = fig.add_subplot(121)
    ax1.plot(pctchange,color=palette[0],linewidth = 3, linestyle=':', label="Percent Change in Sales with +1 Capacity")
    ax1.axhline(0, linewidth = 2,color = palette[-1] ,linestyle='-' )
    ax1.axvline(a, linewidth = 3,color = palette[2], linestyle='--', label="Avg. Observed Capacity" )
    ...
    #ax3.text(1,1,"blah")
    ax3.plot(df22,color=palette[4],linewidth = 3, linestyle='-.', label="Avg. Flight Rev. Given Capacity")
    ...
    laba = Line2D([0], [0], color=palette[0], linewidth=3, linestyle=':')
    labb = Line2D([0], [0], color=palette[2], linewidth=3, linestyle='-')
    labc = Line2D([0], [0], color=palette[4], linewidth=3, linestyle='-.')
    labels = ['Percent Change in Sales with +1 Capacity', 'Avg. Observed Capacity', 'Avg. Flight Revenue Given Capacity']
    lines = [laba,labb,labc]
    L                           = ax1.legend(lines, labels, loc="center right",frameon=True,framealpha=0.8)
    ...
    ax2.plot(df3[:60],"-s", color=palette[1], ms=10, markevery=(len(df3)+1),  linewidth = 3, label="Capacity=" + str(int(a)-6))
    ax2.plot(df4[:60],"--o", color=palette[2], ms=10, markevery=(len(df3)+1),  linewidth = 3, label="Capacity=" + str(int(a)))
    ax2.plot(df5[:60],"-.^", color=palette[0], ms=10, markevery=(len(df3)+1),  linewidth = 3, label="Capacity=" + str(int(dif01)))
    ...
    #
    name = "stochastic_limit_"+market+".pdf"
...
if __name__ == "__main__":
    ...
    StoreX                  =       np.zeros((len(np.arange(1,qBar))*numSim, 61, 11))# f, t, (SeatsRemain, P, Sales, Larrive, Harrive, Lbuy, Hbuy)
    Q0                      =       np.repeat(np.arange(1,qBar),numSim)
    StoreX [:, 0, 0]        =       Q0.astype("int")
    StoreX [:, 0, -1]       =       df_route.loc[df_route.tdate == 0].sample(len(np.arange(1,qBar))*numSim, replace = True)[["dd_dow"]].values.reshape(-1).astype("int")
    #pool2               = Pool(processes=numThreads)
    with Pool(processes=numThreads) as pool2:
        results             = np.array(pool2.map(partial(storeSim, CCP=CCP, gamma=gamma, mu=mu, prices=prices,VAR=VAR), StoreX))
        pool2.close()
        pool2.join()
