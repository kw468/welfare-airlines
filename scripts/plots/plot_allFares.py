"""
    This script plots average fares over time for each
...
for i in range(nrows - 1):
    for j in range(ncols):
        ...
        axs[i, j].plot(df1.groupby("ttdate").fare.mean(),color = PALETTE[4])   
...
for j in range(remain):
    df1 = df.loc[df.route == markets[counter]]
    axs[nrows - 1, j].plot(df1.groupby("ttdate").fare.mean(),color = PALETTE[4])   
    axs[nrows - 1, j].set_ylim((0, 700))
    axs[nrows - 1, j].set_title(markets[counter], fontsize = FONT_SIZE, fontname = FONT)
    counter += 1

for ax in axs.flat:
    ax.set(xlabel = "Booking Horizon", ylabel = "Mean Fare")
...
plt.savefig(
    f"{OUTPUT}/avg_fare_allRoutes.pdf",
    bbox_inches = "tight",
    format = FIG_FORMAT,
    dpi = DPI
)
