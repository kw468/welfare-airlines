"""
    This script calculates alternative stats in
...
df_n.loc[df_n.capF.notnull()][["origin", "dest", "flightNum", "ddate"]].drop_duplicates().shape[0] / \
    df_n[["origin", "dest", "flightNum", "ddate"]].drop_duplicates().shape[0]
#0.5836279867138112
...
modes1 = changeEquip1.groupby(["origin", "dest", "flightNum", "ddate"]) \
    .capacity.agg(pd.Series.mode).reset_index()
...
changeSM = changeEquip1.loc[changeEquip1.indC == 1] \
    .groupby(["origin", "dest", "flightNum", "ddate"]).tdate.max().value_counts()
...
changes = changes.sort_values(
    ["origin", "dest", "flightNum", "ddate", "tdate"],
    ascending = False
).reset_index(drop = True)
changes["difC"] = changes.groupby(["origin", "dest", "flightNum", "ddate"]) \
    .capacity.shift(1)
...
changes["elf"] = changes.groupby(["origin", "dest", "flightNum", "ddate"]) \
    .lf.transform("last")

means = changes[["origin", "dest", "flightNum", "ddate", "elf"]].drop_duplicates() \
    .groupby(["origin", "dest"]).elf.mean().reset_index()
...
a = stats.ttest_rel(
    changes.loc[changes.difC_U == True].elf,
    changes.loc[changes.difC_U == True].avg
)
b = stats.ttest_rel(
    changes.loc[changes.difC_D == True].elf,
    changes.loc[changes.difC_D == True].avg
)
