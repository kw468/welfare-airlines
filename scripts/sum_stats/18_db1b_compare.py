...
path = "/mnt/data0/bts_airline/db1b/cleaned/"
pathResources = "/mnt/data0/bts_airline/resources/"
...
def determineClosebyAirports(files):
    ...
    df                  = pd.read_csv(pathResources + "airports.dat", header=None)
    df["ones"]          = 1
    ...
    cross['distance'] = haversine_np(cross['lon_x'],cross['lat_x'],cross['lon_y'],cross['lat_y'])
...
df.to_csv(pathResources + "db1b_jmp.csv")
...
directPax   = df.loc[df.connect1.isnull()][cols + ["paxx"]].groupby(cols).paxx.sum().reset_index(drop=False)
connectPax  = df.loc[df.connect1.notnull()][cols + ["paxx"]].groupby(cols).paxx.sum().reset_index(drop=False)
ODPax       = df.groupby(cols).paxx.sum().reset_index(drop=False)

result      = directPax.rename(columns = {"paxx" : "directPax"})
result      = result.merge(connectPax.rename(columns = {"paxx" : "connectPax"}), on = cols, how = "left")
result      = result.merge(totalPax.rename(columns = {"paxx" : "totalPax"}), on = cols, how = "left")
result      = result.merge(ODPax.rename(columns = {"paxx" : "odPax"}), on = cols, how = "left")
...
nsCarrier               = nsCarrier.merge(totalPax.loc[totalPax.paxx > .5*50*8*3][cols], on = cols, how = "inner")
nsCarrier               = nsCarrier.groupby(cols + ["mktcar1"])["paxx"].sum().reset_index(drop=False)
...
# rev per quarter
tmp = df.loc[df.connect1.isnull()].merge(result.loc[result.mktcar1.str.len() ==1 ][["origin", "dest", "quarter", "year"]].drop_duplicates())
...
potMarkets = potMarkets.merge(df[["origin", "dest", "nstopdist"]].drop_duplicates(), on = ["origin", "dest"], how = "left")
...
potMarkets.loc[(potMarkets.year == 2019) & (potMarkets.fracDirect > .95) & (potMarkets.nonconnectingTraffic > .95)].mktcar1.str[0].value_counts().sum()
#556
potMarkets.loc[(potMarkets.year == 2019) & (potMarkets.fracDirect > .95) & (potMarkets.nonconnectingTraffic > .95)].mktcar1.str[0].value_counts()
# G4    368
# NK    155
# AS     18
# B6     15
...
OD_hist = OD.groupby(["origin", "dest"])[["nonconnectingTraffic", "fracDirect"]].mean().reset_index(drop=False)
...
distCompare = potMarkets.groupby(["origin", "dest"])[["nonconnectingTraffic", "fracDirect"]].mean().reset_index(drop=False)
...
distCompare["L2"] = np.sqrt(distCompare.fracDirect**2 + distCompare.nonconnectingTraffic**2)
distRoutes = distCompare.merge(dfR, on = ["origin", "dest"], how="right")
D1 = [stats.percentileofscore(distCompare.L1,distRoutes.L1.values[x]) for x in range(len(distRoutes.L1.values))]
D2 = [stats.percentileofscore(distCompare.L2,distRoutes.L2.values[x]) for x in range(len(distRoutes.L2.values))]
np.nanmean(D1)
np.nanmean(D2)
...
distCompare = distCompare.groupby(["origin", "dest"])[["nonconnectingTraffic", "fracDirect"]].mean().reset_index(drop=False)
distCompare["nonconnectingTraffic"] = 1-distCompare.nonconnectingTraffic
distCompare["fracDirect"] = 1-distCompare.fracDirect
distCompare["L1"] = distCompare.fracDirect + distCompare.nonconnectingTraffic
distCompare["L2"] = np.sqrt(distCompare.fracDirect**2 + distCompare.nonconnectingTraffic**2)
...
X = potMarkets.groupby(["origin", "dest"])["nonconnectingTraffic"].mean().reset_index(drop=False)
Y = potMarkets.groupby(["origin", "dest"])["fracDirect"].mean().reset_index(drop=False)
...
plt.scatter(100*Y.fracDirect, 100*X.nonconnectingTraffic, s=1, color=sns.xkcd_palette(colors)[0], label = "DB1B data")
plt.scatter(100*OD_hist.fracDirect, 100*OD_hist.nonconnectingTraffic, s=7, color=sns.xkcd_palette(colors)[3], marker="s", label = "Selected Routes")
plt.xlabel("Nonstop Traffic Percentage")
plt.ylabel("Non-connecting Traffic Percentage")
plt.axvline(100*Y.fracDirect.mean(), linestyle='-.', linewidth=2, color=sns.xkcd_palette(colors)[1], label = "Mean")
plt.axhline(100*X.nonconnectingTraffic.mean(), linestyle='-.', linewidth=2, color=sns.xkcd_palette(colors)[1])

reg = sm.OLS(100*Y.fracDirect,sm.add_constant(100*X.nonconnectingTraffic)).fit()
X_plot = np.linspace(0,100,100)
plt.plot(X_plot, reg.params[0] + X_plot*reg.params[1], linestyle='-', linewidth=2, color=sns.xkcd_palette(colors)[2], label="Regression Line")
plt.legend()

plt.savefig(pathResources + "db1b_routes.pdf",bbox_inches='tight',format= "pdf",dpi=600)
plt.clf()
...
plt.savefig(pathResources + "db1b_routes_small.pdf",bbox_inches='tight',format= "pdf",dpi=600)
plt.clf()
...
tbl_fares = df.loc[df.connect1.isnull()].merge(dfR, on = ["origin", "dest", "year"], how = "inner")
...
TBL1[["directPax", "connectPax", "totalPax", "fare"]] = TBL1[["directPax", "connectPax", "totalPax", "fare"]].astype("int")
TBL1["route"] = TBL1[["origin", "dest"]].min(axis=1) + TBL1[["origin", "dest"]].max(axis=1) 
...
TBL1 = TBL1[["origin", "dest", "directPax_x", "directPax_y", "totalPax_x", "totalPax_y", "connectPax_x", "connectPax_y", "fare_x", "fare_y"]]

tbl_fares1 = df.loc[(df.connect1.notnull()) & (df.connect2.isnull())].merge(dfR, on = ["origin", "dest", "year"], how = "inner")
tbl_fares1.loc[tbl_fares1.year == 2012, "fare"] = tbl_fares1.fare*1.12
tbl_fares1["route"] = tbl_fares1[["origin", "dest"]].min(axis=1) + tbl_fares1[["origin", "dest"]].max(axis=1) 
...
tbl_fares1 = tbl_fares1.groupby(["route"]).fare.sum().astype("int").reset_index(drop=False)
...
with open(pathResources + 'routeSummary.tex', 'w') as tf:
