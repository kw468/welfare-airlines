...
def processASFile(fname):
    ...
    for d in data:
        if d['stops'] == "Nonstop":
            try:
                flightInfo      =       [d['flights'][0]['flight_number'],
                ...
                seatMap         =       pd.concat((seatMap, pd.DataFrame([info]*seatMap.shape[0])),axis=1)
                X               =       X.append(seatMap)
            except KeyError:
...
df                               .to_parquet(path+"seatmap_as_raw.parquet")

df["flight"]                     =   df.groupby(['origin', 'dest', 'flightNum']).ngroup()
df["ddate"]                      =   pd.to_datetime(df['ddate_y']+df['ddate_m']+df['ddate_d'], format='%Y%m%d')
df["sdate"]                      =   pd.to_datetime(df['sdate_y']+df['sdate_m']+df['sdate_d'], format='%Y%m%d') #df.groupby(['sdate_y', 'sdate_m', 'sdate_d']).ngroup()
df["avail"]                      = 0
...
df1                              = pd.pivot_table(df, values='avail', index=['flight', 'sdate', 'ddate', 'row'],
                                                columns=['seatPos'])
...
### UPPER BOUNDS
(df4.exp.sum() - df4.loc[df4["index"] <= 1].exp.sum() - 2*(len(X) - counter)) / df4.exp.sum()
#0.4445254528369139
(df4.dif.sum() - df4.loc[df4["index"] <= 1].dif.sum() - (len(X) - counter)) / df4.loc[( df4["index"]> 0)].dif.sum()
#0.253293820665486
