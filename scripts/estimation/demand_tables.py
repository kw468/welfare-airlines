...
def writeTable(markets,table,name):
    ...
    "\\multicolumn{2}{l}{Number of Flights} &" + \
        "&".join([f1(round(float(a))) for a in table[:, 47]]) + "\\\\",
    "\\multicolumn{2}{l}{Number of Dep. Dates} &" + \
        "&".join([f1(round(float(a))) for a in table[:, 48]]) + "\\\\",
    "\\multicolumn{2}{l}{Number of Obs.} &" + \
        "&".join([f1(round(float(a))) for a in table[:, 49]]) + "\\\\",
    "\\bottomrule",
    "\\end{tabular}",
    ]
    with open(OUTPUT + "param_ests_" + name + ".tex", "w") as tf:
        for z in Z:
            tf.write(z + "\n")
...
np.corrcoef(
    df[[0,1,2,3,4,5,6]].mean().values, df[["ones",16,17,18,19,20,21]].mean().values
)
# array([[ 1.        , -0.84948472],
#        [-0.84948472,  1.        ]])
