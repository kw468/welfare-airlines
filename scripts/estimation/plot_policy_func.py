"""
    This script estimates and plots the policy function
...
# define the log probability of that demand is equal
# to q given the parameters of the model
...
plt.setp(plt.legend().texts, family=FONT, font_size = FONT_SIZE - 2)
plt.xlabel("Seats Remaining", **CSFONT)
plt.ylabel("Price", **CSFONT)
plt.yticks(fontname = FONT, font_size = FONT_SIZE - 2)
plt.xticks(fontname = FONT, font_size = FONT_SIZE - 2)
plt.savefig(
    OUTPUT + "model_policy.pdf",
    bbox_inches = "tight",
    format = FIG_FORMAT,
    dpi = DPI
)
