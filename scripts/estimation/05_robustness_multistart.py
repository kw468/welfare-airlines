...
pathOut     = pathData + "estimation/" + market + "/robust_estim/"
...
xInit = np.genfromtxt(pathIn + market + "_params.csv")
...
pd.DataFrame(solution0).to_csv(pathOut + market + "_robust_params.csv", header=None, index=None)
LLN                     = float(logLike_jit(solution0,data))
tbl                     = [LLN]
pd.DataFrame(np.array(tbl)).to_csv(pathOut + market + "_robust_info.csv", header=None, index=None)
