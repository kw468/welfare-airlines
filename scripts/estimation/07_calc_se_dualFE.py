...
pathOut     = pathData + "estimation/" + market + "/se/"
...
def estimKnitro(VAR,data):
    ...
    KN_set_int_param                (kc, "gradopt", KN_GRADOPT_FORWARD ) #KN_GRADOPT_EXACT,KN_GRADOPT_FORWARD
...
X       = np.array(X)
df      = pd.DataFrame(X)
df.to_csv(pathOut + market+"_bs_samples.csv", header=None, index=None)

SE = np.sqrt(1/len(X) * ((X - X.mean(0))**2).sum(0))
pd.DataFrame(SE).to_csv(pathOut + market+"_se.csv", header=None, index=None)

tstat = xInit/SE
pd.DataFrame(tstat).to_csv(pathOut + market+"_tstat.csv", header=None, index=None)
