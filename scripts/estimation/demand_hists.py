...
def processRoute(num):
    ...
    df = pd.read_csv(
        INPUT + routes[num]  + "/" + routes[num] + ".csv",
        index_col = 0
    )
    prices = np.genfromtxt(
        INPUT + routes[num]  + "/" + routes[num] + "_prices.csv"
    )
    ...
    sig = VAR[-1]
    sl = np.exp(beta[df.dd_dow.values] + bL * prices[df.fareI.values]) / \
        (1 + np.exp(beta[df.dd_dow.values] + bL * prices[df.fareI.values]))
    sb = np.exp(beta[df.dd_dow.values] + bB * prices[df.fareI.values]) / \
        (1 + np.exp(beta[df.dd_dow.values] + bB * prices[df.fareI.values]))
    elas = (bL * prices[df.fareI.values] * (1 - sl)) * \
        (1 - gamma[df.tdate.values]) + \
        (bB * prices[df.fareI.values] * (1-sb)) * \
        gamma[df.tdate.values]
...
def meanSummaryStats(X):
    ...
    a = X1.mean(), X1.std(), np.quantile(X1, .5), \
        np.quantile(X1, .25), np.quantile(X1, .75)
    b = X2.mean(), X2.std(), np.quantile(X2, .5), \
        np.quantile(X2, .25), np.quantile(X2, .75)
    c = X3.mean(), X3.std(), np.quantile(X3, .5), \
        np.quantile(X3, .25), np.quantile(X3, .75)
    d = X4.mean(), X4.std(), np.quantile(X4, .5), \
        np.quantile(X4, .25), np.quantile(X4, .75)
    e = X5.mean(), X5.std(), np.quantile(X5, .5), \
        np.quantile(X5, .25), np.quantile(X5, .75)

    p1 = "DoW Preferences" + \
        " ".join(["&" + str("{0:.2f}".format(f)) for f in a]) + \
        "\\\\[.5ex]"
    p2 = "Leisure Price Sensitivity" + \
        " ".join(["&" + str("{0:.2f}".format(f)) for f in b]) + \
        "\\\\[.5ex]"
    p3 = "Business Price Sensitivity" + \
        " ".join(["&" + str("{0:.2f}".format(f)) for f in c]) + \
        "\\\\[.5ex]"
    p4 = "Prob(Business)" + \
        " ".join(["&" + str("{0:.2f}".format(f)) for f in e]) +  \
        "\\\\[.5ex]"
    p5 = "DoW Arrival Rates" + \
        " ".join(["&" + str("{0:.2f}".format(f)) for f in d]) +  \
        "\\\\[.5ex]"

    with open(OUTPUT + "demand_sum.txt", "w") as f:
...
def calcElas(num):
    ...
    # equivalent to jnp.array([1/(1 + jnp.exp(-g[0] + -t*g[1] - t**2*g[2])) for t in range(0,60)])
    # range(int(min(Tdata)),int(max(Tdata)+1))])
    muT = np.array([VAR[12]] * (T - 20) + \
        [VAR[13]] * 7 + [VAR[14]] * 7 + [VAR[15]] * 6)
    muD = np.append(np.array([1]), np.array(VAR[16:22]))
    mu = muT[:, None] * muD[None, :]
    sig = VAR[-1]
    sl = np.exp(beta[df.dd_dow.values] + bL * prices[df.fareI.values]) / \
    ...
    sb = np.exp(beta[df.dd_dow.values] + bB * prices[df.fareI.values]) / \
        (1 + np.exp(beta[df.dd_dow.values] + bB * prices[df.fareI.values]))
    elas = (bL * prices[df.fareI.values] * (1 - sl)) * \
        (1 - gamma[df.tdate.values]) + (bB * prices[df.fareI.values] * \
        (1 - sb)) * gamma[df.tdate.values]
...
def wtp(num):
    ...
    prices = np.genfromtxt(INPUT + routes[num]  + "/" + routes[num] + "_prices.csv")
    beta = np.array(VAR[0:7])
    bL = np.minimum(VAR[7], VAR[8])
    bB = np.maximum(VAR[7], VAR[8])
    gamma = 1 / (np.exp(
