...
# q given the parameters of the modelt
def log_demandQ(beta, bL, bB, gamma, mu, prices, q):
    return q * jnp.log(mu[:, None, :]) + q * \
        jnp.log(purchIn(beta, bB, bL, gamma, prices)) - \
        (mu[:, None, :] * purchIn(beta, bB, bL, gamma, prices)) - \
        gammaln(q + 1)

# define the probability of that demand is equal to
# q given the parameters of the model, this is just exp(log(p)),
    ...
    for t in range(1, T + 1):
        ...
        for b in range(len(beta)):
            if t != T: # update expected value function, this is for not the last period
                ...
                EV = EV.at[-t, :, :, b] \
                    .set(jnp.sum(g * V[:, -t, b][None, :, None], axis = 1) * Pt[-t, 1:])
        if t != 1:
            XX = (ER[:, -t, :, :] + EV[-t + 1, :, :, :]) / \
...
def process(market):
    ...
    gamma = 1 / (jnp.exp(
        -VAR[9] - jnp.arange(0, 60) * VAR[10] - (jnp.arange(0, 60) ** 2) * \
            VAR[11]) + 1
    )
    ...
    df["prices_sim_Pt"] = (
        100 * np.exp(
            CCP[df_route.tdate.values, df_route.seats.values, :, df_route.dd_dow.values]
        ) * prices[None, :]
    ).sum(axis = 1)
    df["prices_sim_P1"] = (
        100 * np.exp(
            CCP1[df_route.tdate.values, df_route.seats.values, :, df_route.dd_dow.values]
        ) * prices[None, :]
    ).sum(axis = 1)
...
plt.plot(
    df.groupby("tdate").prices_actual.mean(),
    label = "Mean: Data",
    color = PALETTE[2],
...
plt.plot(
    df.groupby("tdate").prices_sim_Pt.mean(),
    label = "Mean: Model",
    color = PALETTE[0],
    linewidth = LINE_WIDTH,
    linestyle = "--"
...
plt.plot(
    df.groupby("tdate").prices_sim_P1.mean(),
    label = "Mean: Model, no fare restrictions",
    color = PALETTE[3],
    linewidth = LINE_WIDTH,
    linestyle = ":"
)
plt.plot(
    df.groupby("tdate").prices_actual.quantile(.95),
    label = "5th,95th Percentiles: Data",
    color = PALETTE[2],
    linewidth = LINE_WIDTH - 2,
    linestyle = "-"
)
plt.plot(
    df.groupby("tdate").prices_sim_P1.quantile(.95),
    label = "5th,95th Percentiles: Model",
    color = PALETTE[0],
    linewidth = LINE_WIDTH - 2,
    linestyle = "--"
)
plt.plot(
    df.groupby("tdate").prices_actual.quantile(.05),
    label = "_nolegend_",
    color = PALETTE[2],
    linewidth = LINE_WIDTH - 2,
    linestyle = "-"
...
plt.plot(
    df.groupby("tdate").prices_sim_P1.quantile(.05),
    label = "_nolegend_",
    color = PALETTE[0],
    linewidth = LINE_WIDTH - 2,
    linestyle="--"
...
#     #
#     name = "model_fit.pdf"
#     plt.savefig(OUTPUT + name,bbox_inches="tight",format= "pdf",dpi=600)
#     plt.close()
