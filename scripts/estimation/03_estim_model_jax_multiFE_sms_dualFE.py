...
def demandQ(beta, bL, bB, gamma, mu, q):
    ...
    # (mu[:,None]*purchIn(aB,bB,aL,bL,prices,gamma))**q*jnp.exp(-mu[:,None]*(purchIn(aB,bB,aL,bL,prices,gamma)))/factQ[q]
...
def allDemand(beta, bL, bB, gamma, mu):
    ...
    for q in range(1,qBar):         # seats remaining
        ...
        # define the probability of selling out as 1 - prob of not selling out, ensuring probabilities sum to 1
        f                           = f.at[q,q,:,:,:].set(jnp.maximum(1 - jnp.sum(f[q,0:(q),:,:,:],axis=0),1e-100))
    f                               = jnp.where((f > 0) & (f < 1e-100), 1e-100, f) # if any small floats are created, limit them to 1e-100
...
def dynEst(f,ER,gamma,sig,beta):
    ...
    for t in range(1,T+1):
        ...
        if t == 1:
            ...
            # CCP[-t,:,:] = Pt[-t,1:][None,:]*np.exp(ER[:,-t,:]/sig)/np.sum(np.exp(ER[:,-t,:]*Pt[-t,1:][None,:]/sig),axis=1)[:,None]
        else:
...
def logLike(VAR, data):
    ...
    loss0               =       jnp.sum(jnp.log(f0[data[:,0], data[:,1], data[:,2], data[:,3], data[:,4] ]))
...
def estimKnitro(VAR,data,speed):
    n = len(VAR)
    bndsLo = np.array ([ -10,-10,-10,-10,-10,-10,-10,-10,-10,-250,-10,-.06,.1 ,.1 ,.1 ,.1 ,.01,.01,.01,.01,.01,.01,.02])
    bndsUp = np.array ([  15 ,15,15 , 15,15 ,15,15 , 0,  0,40  ,10 , .15,15 ,15 ,15 ,15 ,15,15,15,15,15,15,2])
    kc = KN_new()
    KN_add_vars                     (kc, n)
    ...
    KN_set_int_param                (kc, "algorithm", KN_ALG_ACT_SQP) #KN_ALG_ACT_SQP KN_ALG_BAR_DIRECT
    ...
    KN_set_int_param                (kc, "gradopt", KN_GRADOPT_EXACT ) #KN_GRADOPT_EXACT,KN_GRADOPT_FORWARD
    KN_set_double_param             (kc, KN_PARAM_FEASTOL, 1.0E-8 )
    KN_set_double_param             (kc, KN_PARAM_OPTTOL, 1.0E-8) 
    KN_set_int_param                (kc, "par_numthreads", numThreads )
    KN_set_int_param                (kc, "blasoption", 1 )
    KN_set_int_param                (kc, "par_blasnumthreads", numThreads )
...
df                      = pd.read_parquet(pathData + "asdata_clean.parquet")
...
while success == False:
    k                   = it
    kmeans              = KMeans(n_clusters=k, random_state=0).fit(df_route.fare.values.reshape(-1, 1))
...
EC                      = 0.5772156649
xInit                   = [  2.49999999 ,2.49999999,2.49999999 ,2.49999999,2.49999999 ,\
...
df_route.to_csv(pathOut + market + ".csv")
pd.DataFrame(solution0).to_csv(pathOut + market + "_params.csv", header=None, index=None)
pd.DataFrame(np.array(Pt)).to_csv(pathOut + market + "_Pt.csv", header=None, index=None)
pd.DataFrame(np.array(prices)).to_csv(pathOut + market + "_prices.csv", header=None, index=None)

LLN                     = float(logLike_jit(solution0,data))
tbl                     = [LLN, numFlights, numDDates, numObs]
pd.DataFrame(np.array(tbl)).to_csv(pathOut + market + "_info.csv", header=None, index=None)
