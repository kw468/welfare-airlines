...
df                              = pd.read_parquet(pathData + "efdata_clean.parquet")
...
# if market is BOS_MCI, then early periods also had Frontier as a nonstop carrier. Remove these entries.
if market == "BOS_MCI": 
    df_route                    = df_route.loc[df.ddate >= "2012-05-17"] # remove entries with Frontier

df_route                        = df_route.loc[df_route.difS.notnull()] # keep non-null obs
